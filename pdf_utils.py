...
def generate_payment_history_pdf(mieter_name, jahr, history_data, h_stats):
    """Diese Funktion hat im Import gefehlt"""
    pdf = FPDF()
    pdf.add_page()
    
    # Header
    pdf.set_font("Helvetica", '', 8)
    abs_line = f"{h_stats.get('name', '')}, {h_stats.get('street', '')}, {h_stats.get('city', '')}"
    pdf.cell(0, 5, abs_line, ln=True)
    pdf.ln(10)
