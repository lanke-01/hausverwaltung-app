...
if conn:
    # --- DATENBANK-STRUKTUR PRÜFEN / ERWEITERN ---
    # Wir machen das in einem separaten Block, um Transaction-Fehler zu vermeiden
    try:
        ...
        conn.rollback() # Wichtig: Transaktion zurücksetzen, falls es hakt
        st.error(f"Fehler bei Datenbank-Update: {e}")
    ...
    try:
        ...
        if not data:
            # Falls gar kein Eintrag existiert, einen anlegen
            cur.execute("INSERT INTO landlord_settings (id, name, total_area) VALUES (1, 'Vermieter Name', 0.00)")
            conn.commit()
            cur.execute("SELECT name, street, city, iban, bank_name, total_area FROM landlord_settings WHERE id = 1")
            data = cur.fetchone()
    except Exception as e:
