    ...
    try:
        ...
        if tenants_data:
            ...
            with tab1:
                ...
                if t_row:
                    st.subheader(f"Zahlungsverlauf {jahr}")
                    ein = t_row[2].strftime('%d.%m.%Y') if t_row[2] else "unbekannt"
                    aus = t_row[3].strftime('%d.%m.%Y') if t_row[3] else "laufend"
                    ...
                    # --- BUTTON FÜR TAB 1 ---
                    if st.button("🖨️ PDF Kontoauszug erstellen", key="btn_pay_pdf"):
                        ...
                        with open(path, "rb") as f:
                            st.download_button("💾 Download Kontoauszug", f, file_name=os.path.basename(path), key="dl_pay_pdf")
            ...
            with tab2:
                ...
                if h_row and m_row:
                    ...
                    voraus_gesamt = float(m_row[4]) * (tage_mieter / 30.43)
                    saldo = summe_mieter - voraus_gesamt
                    
                    st.metric("Saldo", f"{saldo:.2f} €", delta_color="inverse")

                    if st.button("🖨️ PDF Abrechnung erstellen", key="btn_nk_pdf"):
                        h_stats = {"name": h_row[0], "street": h_row[1], "city": h_row[2], "iban": h_row[3], "bank": h_row[4]}
                        path = generate_nebenkosten_pdf(f"{m_row[6]} {m_row[7]}", str(m_row[5]), z_raum, tage_mieter, pdf_rows, summe_mieter, voraus_gesamt, saldo, {}, h_stats)
                        with open(path, "rb") as f:
                            st.download_button("📩 Download Abrechnung", f, file_name=os.path.basename(path), key="dl_nk_pdf")
